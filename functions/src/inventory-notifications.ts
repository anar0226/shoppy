 ...
 */
async function checkStoreInventory(storeId: string, storeData: any) {
  ...
  for (const productDoc of productsSnapshot.docs) {
    ...
    try {
      ...
      const productAlerts = generateProductAlerts(
        storeId,
        productId,
        product,
        stockLevel,
 ...
 */
function calculateTotalStock(product: any): number {
  if (product.variants && product.variants.length > 0) {
    ...
    for (const variant of product.variants) {
      if (variant.trackInventory && variant.stockByOption) {
        const variantStock = Object.values(variant.stockByOption as Record<string, number>)
 ...
 */
function generateProductAlerts(
  storeId: string,
  productId: string,
  product: any,
  stockLevel: number,
 ...
 */
async function checkReorderAlerts(storeId: string): Promise<InventoryAlert[]> {
  const alerts: InventoryAlert[] = [];
 ...
 */
async function sendInventoryNotifications(
  storeId: string,
  ownerId: string,
 ...
 */
function groupAlertsByType(alerts: InventoryAlert[]): Record<string, InventoryAlert[]> {
  const groups: Record<string, InventoryAlert[]> = {};
 ...
 */
function createNotificationFromAlerts(
  storeId: string,
  ownerId: string,
  alertType: string,
  alerts: InventoryAlert[]
 ...
 */
async function checkRecentNotification(storeId: string, alertType: string): Promise<boolean> {
 ...
 */
async function sendPushNotification(ownerId: string, criticalAlerts: InventoryAlert[]) {
  try {
 ...
 */
export const triggerInventoryCheck = functions.https.onCall(async (data, context) => {
 ...
 */
export const updateInventorySettings = functions.https.onCall(async (data, context) => {
  // Verify user is authenticated
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
function validateInventorySettings(settings: any): any {
 ...
 */
export const getInventoryAnalytics = functions.https.onCall(async (data, context) => {
  // Verify user is authenticated
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
 ...
 */
function processInventoryAnalytics(auditDocs: admin.firestore.QueryDocumentSnapshot[]): any {
  const analytics = {
    totalAdjustments: 0,
    ...
    dailyAdjustments: {} as Record<string, number>,
    topAdjustedProducts: {} as Record<string, { name: string; count: number; totalAdjustment: number }>
  };
