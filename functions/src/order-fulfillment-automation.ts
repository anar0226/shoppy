  ...
  .onUpdate(async (change, context) => {
    ...
    try {
      ...
      await logStatusTransition({
        ...
        reason: afterData.lastTransition?.reason || 'Status updated',
        automated: afterData.lastTransition?.automated || false,
        timestamp: admin.firestore.FieldValue.serverTimestamp() as admin.firestore.Timestamp,
        userId: afterData.lastTransition?.userId,
      });
 ...
 */
export const processAutomaticOrderTransitions = functions.pubsub
...
async function processProcessingOrders() {
  try {
    ...
    for (const doc of processingOrders.docs) {
      const orderData = doc.data() as OrderData;
      const processingStartTime = orderData.processingStartedAt?.toDate() || orderData.updatedAt.toDate();
...
async function processDeliveryOrders() {
  try {
    ...
    for (const doc of deliveryRequestedOrders.docs) {
      await checkDeliveryStatus(doc.id, doc.data() as OrderData);
    }
    ...
    for (const doc of driverAssignedOrders.docs) {
      await escalateDeliveryIssue(doc.id, doc.data() as OrderData, 'Driver pickup delay');
    }
 ...
 */
async function logStatusTransition(transition: StatusTransition) {
  try {
 ...
 * Handle status-specific actions
 */
async function handleStatusSpecificActions(orderId: string, orderData: OrderData) {
 ...
 */
async function scheduleCompletionCheck(orderId: string) {
  try {
 ...
 */
async function checkPaymentStatus(orderId: string, orderData: OrderData) {
  try {
 ...
 */
async function simulatePaymentCheck(paymentIntentId: string): Promise<boolean> {
 ...
 */
async function reserveOrderInventory(orderId: string, orderData: OrderData) {
  try {
 ...
 */
async function releaseOrderInventory(orderId: string, orderData: OrderData) {
  try {
    const items = orderData.items || [];
 ...
 */
async function requestDelivery(orderId: string, orderData: OrderData) {
  try {
 ...
 */
async function checkDeliveryStatus(orderId: string, orderData: OrderData) {
  try {
 ...
 */
async function escalateDeliveryIssue(orderId: string, orderData: OrderData, issue: string) {
  try {
 ...
 */
async function getStoreAverageProcessingTime(storeId: string): Promise<number> {
  try {
    ...
    if (recentOrders.empty) return 30; // Default 30 minutes
 ...
 */
async function sendStatusUpdateNotifications(orderId: string, orderData: OrderData) {
  try {
    switch (orderData.status) {
      case 'paid':
 ...
 */
async function sendOrderConfirmationNotification(orderId: string, orderData: OrderData) {
  console.log(`Sending order confirmation for ${orderId}`);
  // TODO: Implement actual notification sending
}

async function sendProcessingNotification(orderId: string, orderData: OrderData) {
  console.log(`Sending processing notification for ${orderId}`);
}

async function sendReadyNotification(orderId: string, orderData: OrderData) {
  console.log(`Sending ready notification for ${orderId}`);
}

async function sendInTransitNotification(orderId: string, orderData: OrderData) {
  console.log(`Sending in transit notification for ${orderId}`);
}

async function sendDeliveredNotification(orderId: string, orderData: OrderData) {
  console.log(`Sending delivered notification for ${orderId}`);
}

async function sendCancellationNotification(orderId: string, orderData: OrderData) {
  console.log(`Sending cancellation notification for ${orderId}`);
}

async function notifyStoreOwner(orderId: string, orderData: OrderData, message: string) {
  console.log(`Notifying store owner for order ${orderId}: ${message}`);
}

async function notifyCustomer(orderId: string, orderData: OrderData, message: string) {
  console.log(`Notifying customer for order ${orderId}: ${message}`);
}

async function notifySupport(orderId: string, issue: string) {
 ...
 */
async function updateOrderAnalytics(orderId: string, orderData: OrderData) {
  try {
    const analyticsData = {
      orderId,
...
export const updateOrderStatus = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (orderData.storeId) {
      ...
      const isStoreOwner = storeDoc.exists && storeDoc.data()?.ownerId === context.auth.uid;
 ...
 */
export const getOrderFulfillmentMetrics = functions.https.onCall(async (data, context) => {
  // Verify user is authenticated
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
  ...
  try {
    ...
    for (const doc of orders.docs) {
      ...
      const deliveryRequestedAt = data.deliveryRequestedAt?.toDate();
