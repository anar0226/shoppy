...
exports.ubcabWebhook = exports.qpayWebhook = void 0;
 ...
 */
exports.qpayWebhook = firebase_functions_1.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        if (!verifyQPaySignature(webhookData, req.headers['x-qpay-signature'])) {
 ...
 */
exports.ubcabWebhook = firebase_functions_1.https.onRequest(async (req, res) => {
    // Verify the request method
    if (req.method !== 'POST') {
        res.status(405).send('Method Not Allowed');
        return;
    ...
    try {
        ...
        if (!verifyUBCabSignature(webhookData, req.headers['x-ubcab-signature'])) {
 ...
 */
function verifyQPaySignature(data, signature) {
    try {
 ...
 */
function verifyUBCabSignature(data, signature) {
    try {
 ...
 */
async function processQPayWebhook(data) {
    try {
 ...
 */
async function processUBCabWebhook(data) {
    try {
        ...
        // Send notification to customer
        const orderData = orderDoc.data();
        await sendDeliveryNotification(orderData === null || orderData === void 0 ? void 0 : orderData.userId, status, {
 ...
 */
async function triggerDeliveryRequest(orderId, orderData) {
    try {
 ...
 */
async function sendPaymentNotification(userId, type, data) {
    try {
 ...
 */
async function sendDeliveryNotification(userId, status, data) {
    try {
        const notificationData = {
            userId,
 ...
 */
function getDeliveryNotificationTitle(status) {
    switch (status.toLowerCase()) {
        case 'driver_assigned':
 ...
 */
function getDeliveryNotificationMessage(status, data) {
    switch (status.toLowerCase()) {
        case 'driver_assigned':
